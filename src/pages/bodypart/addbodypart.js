...
import { addBodypart } from 'src/NetworkUtils/NetworkUtils';
...
const BodypartDetails = (props) => {
    ...
    function onSave() {
        if (formik.values.title !== '' && imageurl !== '') {
            setisloading(true)
            addBodypart(formik.values.title, imageurl).then((res) => {
                if (res.code === 200) {
                    setisloading(false)
                    history.back();
                }
...
BodypartDetails.getLayout = (page) => (
    <DashboardLayout>
        {page}
    </DashboardLayout>
);
